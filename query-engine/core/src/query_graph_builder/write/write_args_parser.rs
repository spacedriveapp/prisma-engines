...
fn parse_composite_update_many(
    ...
    let where_map: ParsedInputMap<'_> = value.shift_remove(args::WHERE).unwrap().try_into()?;
    let filter = extract_filter(where_map, cf.typ())?;

    let update_map: ParsedInputMap<'_> = value.shift_remove(args::DATA).unwrap().try_into()?;
...
fn parse_composite_upsert(
    ...
    let set = value.shift_remove(operations::SET).unwrap();
    let set = parse_composite_writes(cf, set, path)?.try_into_composite().unwrap();
    let update: ParsedInputMap<'_> = value.shift_remove(operations::UPDATE).unwrap().try_into()?;
