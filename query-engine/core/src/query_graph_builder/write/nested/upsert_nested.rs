...
pub fn nested_upsert(
    ...
    for value in coerce_vec(value) {
        ...
        let create_input = as_map.shift_remove(args::CREATE).expect("create argument is missing");
        let update_input = as_map.shift_remove(args::UPDATE).expect("update argument is missing");
        let where_input = as_map.shift_remove(args::WHERE);
